...
from api.outcoming import sync_add_employee
...
def add_employee(request):
    ...
    try:
        ...
        for item in EmployeeGender.objects.all():
            sex.append({"id": item.public_code, "name_ru": item.name_ru})
        for item in EmployeeRole.objects.all():
            roles.append({"id": item.public_code, "name_ru": item.name_ru})
        for item in EmployeePosition.objects.all():
            positions.append({"id": item.public_code, "name_ru": item.name_ru})
        for item in Industry.objects.all():
            industries.append({"id": item.public_code, "name_ru": item.name_ru})
            ...
            employees_arr.append(
                {
                    ...
                    "created_at": timezone.localtime(employee.created_at).strftime(
                        "%d.%m.%Y %H:%M:%S"
                    ),
                    "created_by_email": created_by_email,
                }
            )
        response = {"data": list(employees_arr)}
...
def delete_employee(request):
    if request.method == "POST":
        ...
        if employee.user is not None:
            employee_created_participant = Participant.objects.filter(
...
def save_new_employee_xls(request):
    if request.method == "POST":
        ...
        if len(existing_employees) > 0:
            ...
            emails = "None"
        response = {"emails": emails, "cnt": cnt}
        return JsonResponse(response)
    ...
    if request.method == "POST":
        ...
        if check_passed:
            ...
            if "role_id" in employee_data and employee_data["role_id"] != "":
                employee_inst.role = EmployeeRole.objects.get(
                    public_code=employee_data["role_id"]
                )
            if "position_id" in employee_data and employee_data["position_id"] != "":
                employee_inst.position = EmployeePosition.objects.get(
                    public_code=employee_data["position_id"]
                )
            if "industry_id" in employee_data and employee_data["industry_id"] != "":
                employee_inst.industry = Industry.objects.get(
                    public_code=employee_data["industry_id"]
                )
            if "gender" in employee_data and employee_data["gender"] != "":
                employee_inst.sex = EmployeeGender.objects.get(
                    public_code=employee_data["gender"]
                )

            if (
                "employee_birth_year" in employee_data
                and employee_data["employee_birth_year"] != ""
            ):
