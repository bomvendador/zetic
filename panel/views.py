...
def info_common(request):
    ...
    context = {"cur_userprofile": userprofile}
...
def home(request):
    ...
    individual_reports = ReportData.objects.filter(
        report__participant__created_by=request.user
    )
    for report in individual_reports:
        # print(report.report.participant.employee.name)
        if report.section_code == "1":
            points_1[report.category_code] = (
                points_1[report.category_code] + report.points
            )
        if report.section_code == "2":
            points_2[report.category_code] = (
                points_2[report.category_code] + report.points
            )
        if report.section_code == "3":
            points_3[report.category_code] = (
                points_3[report.category_code] + report.points
            )
        if report.section_code == "4":
            points_4[report.category_code] = (
                points_4[report.category_code] + report.points
            )
    ...
    stats = {
        "total_completion_time": str(timedelta(seconds=total_completion_time)).split(
            "."
        )[0],
    ...
    if context == "logout":
        return render(
            request, "login.html", {"error": "Ваша учетная запись деактивирована"}
        )
    else:
        userprofile = UserProfile.objects.get(user=request.user)
        if userprofile.role.name == "Админ заказчика":
            ...
            stats.update(
                {
                    ...
                    "individual_reports_qnt": Report.objects.filter(
                        study__company=company
                    ).count(),
                    "group_reports_qnt": ReportGroup.objects.filter(
                        company=company
                    ).count(),
                }
            )
            ...
            stats.update(
                {
                    ...
                    "employees_qnt": Employee.objects.filter(
...
def get_company_participants(request):
    if request.method == "POST":
        ...
        participants_inst = Participant.objects.filter(
            employee__company__name=company, completed_at__isnull=False
        )
        participants = []
    ...
    if request.method == "POST":
        ...
        for participant in report_participants:
            ...
            report = Report.objects.filter(
                participant__employee__email=participant
            ).latest("added")
            lie_points.append(
                {
        ...
        response = {"data": list(participants_data), "lie_points": list(lie_points)}
    ...
    if context == "logout":
        ...
        if cur_user_role_name == "Админ заказчика":
            companies = Company.objects.filter(
                id=Employee.objects.get(user=request.user).company.id
            )
        ...
        context.update({"companies_arr": companies_arr})
...
def get_group_reports_list(request):
    if request.method == "POST":
        ...
        for group_report in group_reports:
            ...
            reports_group_square = ReportGroupSquare.objects.filter(
                report_group=group_report
            )
            for report_group_square in reports_group_square:
                report_group_square_arr.append(
                    report_group_square.report.participant.employee.name
                )
            report.append(
                {
                    "company": group_report.company.name,
                    "id": group_report.id,
                    "date": timezone.localtime(group_report.added).strftime(
                        "%d.%m.%Y %H:%M:%S"
                    ),
            ...
            report_arr.append(
                {
                    ...
                    "date": timezone.localtime(report.added).strftime(
                        "%d.%m.%Y %H:%M:%S"
                    ),
...
def send_individual_report(request):
    if request.method == "POST":
        ...
        full_path = os.path.join(
            settings.MEDIA_ROOT, "reportsPDF", "single", report.file.name
        )
        print(full_path)
        sent = False
...
def users_list(request):
    context = info_common(request)
    context.update({"user_profiles": UserProfile.objects.all()})
                ...
                for participant in participants:
                    if "report" in participant:
                        ...
                        if Employee.objects.filter(
                            ...
                            employee_inst = Employee.objects.get(
                                email=participant_info["email"]
                            )
                        else:
                            employee_inst = Employee()
                            ...
                            employee_inst.sex = EmployeeGender.objects.get(
                                public_code=participant_info["sex"]
                            )
                        ...
                        if not Participant.objects.filter(
                            employee__email=participant_info["email"], study=study_inst
                        ).exists():
                            ...
                            participant_inst.total_questions_qnt = int(
                                participant["total_questions_qnt"]
                            )
                            participant_inst.answered_questions_qnt = int(
                                participant["answered_questions_qnt"]
                            )
                            participant_inst.current_percentage = (
                                int(participant["total_questions_qnt"])
                                / int(participant["answered_questions_qnt"])
                                * 100
                            )
