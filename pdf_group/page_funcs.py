...
from pdf_group.draw import draw_arrow
...
def data_by_points(square_results, section_code, category_code):
    ...
    for square_result in square_results:
        ...
        report = Report.objects.filter(
            participant__employee__email=square_result[1]
        ).latest("added")
        if ReportData.objects.filter(
            report=report, section_code=section_code, category_code=category_code
        ).exists():
            print(
                f"участик - {report.participant.employee.name} секция - {section_code} категория - {category_code}"
            )
            report_data = ReportData.objects.get(
                report=report, section_code=section_code, category_code=category_code
            )
            ...
            scale_data.append(
                [square_result[2], report_data.points, cnt, group_color, email, bold]
            )
