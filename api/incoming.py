...
def participant_started(request):
    ...
    # print(json_request)
    print(
        f'{timezone.localtime(timezone.now()).strftime("%d.%m.%Y %H:%M:%S")} - participant_started - {json_request}'
    )
...
def questions_answered_qnt(request):
    ...
    json_request = json.loads(request.body.decode("utf-8"))
    print(
        f'{timezone.localtime(timezone.now()).strftime("%d.%m.%Y %H:%M:%S")} - questions_answered_qnt - {json_request}'
    )
    study_public_code = json_request["study"]["public_code"]
    total_questions_qnt = json_request["study"]["total_questions_qnt"]
...
def companies_employees(request):
    ...
    for company in companies_:
        ...
        for employee in employees:
            employees_arr.append({"name": employee.name, "email": employee.email})
